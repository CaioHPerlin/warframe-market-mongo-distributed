...
export default function PlayerPage() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            api.get(`/api/ratings?player_id=${id}`),
            api.get(`/api/players/${id}/orders`),
        ])
            .then(([p, r, o]) => {
            setPlayer(p);
            setRatings(r);
    ...
    if (loading) {
        return (<div className="max-w-lg mx-auto mt-8 space-y-4">
        <Skeleton className="h-32 rounded-xl"/>
        <Skeleton className="h-48 rounded-xl"/>
      </div>);
    }
    if (!player)
        return <p className="text-destructive text-center py-12">Player not found</p>;
    const positive = player.reputation.positive || 0;
    const negative = player.reputation.negative || 0;
    const total = positive + negative;
    return (<div className="max-w-lg mx-auto mt-8 space-y-6">
      <Card>
      ...
      <Tabs defaultValue="ratings">
        ...
        <TabsContent value="ratings" className="mt-4">
          {ratings.length === 0 ? (<p className="text-muted-foreground text-sm">No ratings yet.</p>) : (<div className="flex flex-col gap-2">
              {ratings.map((r) => (<Card key={r._id} size="sm">
        ...
        <TabsContent value="orders" className="mt-4">
          {orders.length === 0 ? (<p className="text-muted-foreground text-sm">No orders yet.</p>) : (<Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {orders.map((o) => (<TableRow key={o._id}>
                    <TableCell>
