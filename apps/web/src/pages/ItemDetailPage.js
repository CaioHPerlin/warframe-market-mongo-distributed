...
export default function ItemDetailPage() {
    ...
    if (!item)
        return <p className="text-destructive text-center py-12">Item not found</p>;
    ...
    return (<div>
      ...
      <Tabs defaultValue="sell">
        ...
        <TabsContent value="sell" className="mt-4">
          {sellOrders.length === 0 ? (<p className="text-muted-foreground text-sm">No sell orders.</p>) : (<div className="flex flex-col gap-2">
              {sellOrders.map((o) => (<OrderCard key={o._id} order={o} itemName={itemName}/>))}
            </div>)}
        </TabsContent>
        <TabsContent value="buy" className="mt-4">
          {buyOrders.length === 0 ? (<p className="text-muted-foreground text-sm">No buy orders.</p>) : (<div className="flex flex-col gap-2">
              {buyOrders.map((o) => (<OrderCard key={o._id} order={o} itemName={itemName}/>))}
            </div>)}
        </TabsContent>
      </Tabs>
...
function OrderCard({ order, itemName }) {
    return (<Card size="sm">
      <CardContent className="flex items-center justify-between gap-4 pt-(--card-spacing)">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Link to={`/players/${order.player_id}`} className="text-xs text-muted-foreground no-underline hover:text-foreground">
            {order.player_username}
          </Link>
          <WhisperButton username={order.player_username} itemName={itemName} orderType={order.order_type} platinum={order.platinum}/>
        </div>
      </CardContent>
    </Card>);
}
//# sourceMappingURL=ItemDetailPage.js.map