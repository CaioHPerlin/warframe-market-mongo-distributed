...
export default function ItemsPage() {
    ...
    useEffect(() => {
        ...
        api.get(`/api/items${params}`).then(setItems).catch(() => { }).finally(() => setLoading(false));
    }, [search]);
    return (<div>
      ...
      {loading ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        </div>) : items.length === 0 ? (<p className="text-muted-foreground text-center py-12">No items found.</p>) : (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {items.map((item) => (<Link key={item._id} to={`/items/${item._id}`} className="no-underline">
              <Card size="sm" className="hover:border-ring transition-colors cursor-pointer h-full">
                <CardContent className="flex flex-col gap-1.5 pt-(--card-spacing)">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {item.tags?.slice(0, 4).map((tag) => (<Badge key={tag} variant="secondary" className="text-[10px]">{tag}</Badge>))}
                  </div>
                </CardContent>
              </Card>
            </Link>))}
